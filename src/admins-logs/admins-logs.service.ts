...
export class AdminsLogsService {
  ...
  async logAction(
    ...
    try {
      ...
      console.error(`[AdminsLogsService] Failed to record admin log:`, err);
      this.logger.error(
  ...
  async getActivitySummary(days: number = 30) {
    try {
      ...
      console.error('[AdminsLogsService] Failed to calculate activity summary:', err);
      this.logger.error(
  ...
  async getAdminSummary(adminId: string, days?: number) {
    try {
      ...
      console.error(`[AdminsLogsService] Error fetching admin summary for ${adminId}:`, err);
      this.logger.error(`Error fetching admin summary for ${adminId}: ${err?.message || err}`);
      throw new InternalServerErrorException(
        `Failed to retrieve admin summary: ${err?.message || err}`,
      );
    }
  }
  ...
  async findAll({
    ...
    try {
      ...
      console.error('[AdminsLogsService] Error in findAll logs query:', err);
      this.logger.error(`Error in findAll logs query: ${err?.message || err}`);
      throw new InternalServerErrorException(
        `Failed to fetch admin logs: ${err?.message || err}`,
      );
    }
  }
  ...
  async findOne(id: string) {
    try {
      const log = await this.prisma.admins_logs.findUnique({
        where: { id },
        include: {
          staff_accounts: {
            select: {
      ...
      console.error(`[AdminsLogsService] Error fetching log #${id}:`, err);
      this.logger.error(`Error fetching log #${id}: ${err?.message || err}`);
      throw new InternalServerErrorException(
        `Failed to retrieve log details: ${err?.message || err}`,
      );
    }
  }
}
